// LifeOS - Contacts Module v6.0 (Project PHOENIX - High-Fidelity & Tactical UI)
...
LifeOS.contacts = {
    ...
    renderLayout: function() {
        ...
        container.innerHTML = `<div class="search-filters"><div class="search-box"><i class="fas fa-search search-icon"></i><input type="text" class="search-input" id="contacts-search" placeholder="ابحث في الملفات..."></div><div class="view-mode-toggle"><button class="btn btn-small" data-view="dossier" title="نمط الملفات"><i class="fas fa-th-large"></i></button><button class="btn btn-small" data-view="list" title="نمط القائمة"><i class="fas fa-list"></i></button></div><button class="btn btn-success"><i class="fas fa-plus"></i> إضافة ملف</button><button class="btn btn-secondary import-btn"><i class="fas fa-upload"></i> استيراد CSV</button><input type="file" class="import-input" accept=".csv" style="display: none;"></div><div class="filters-wrapper"><div class="filters-header" onclick="toggleContactsFilters()"><h4><i class="fas fa-filter"></i> التصنيفات</h4><button class="filters-toggle"><i class="fas fa-chevron-down"></i></button></div><div class="filters-content expanded"><div class="filter-tags-container" id="contacts-filters"></div></div></div><div id="contacts-grid"></div><div class="text-center" id="contacts-empty" style="display: none;"><i class="fas fa-address-book" style="font-size: 3rem; color: var(--text-muted); margin-bottom: 1rem;"></i><p style="color: var(--text-secondary);">لا توجد ملفات. ابدأ بإضافة ملف جديد أو استيراد.</p></div>`;
    ...
    createCard: function(item) {
        ...
        let orgHTML = ''; if (item.organization?.name) { orgHTML = `<div style="color:var(--text-secondary);font-size:0.9rem;">${LifeOS.core.sanitize(item.organization.name)}${item.organization.title?`, ${LifeOS.core.sanitize(item.organization.title)}`:''}</div>`; }
        
        card.innerHTML = `<div class="card-content"><div class="card-header" style="align-items:center;gap:1rem;">${photoHTML}<div style="flex:1;"><h3 class="card-title">${LifeOS.core.sanitize(item.name||'غير محدد')}</h3>${orgHTML}</div></div><div id="collapsible-${item.id}" class="contact-details" style="margin:1.5rem 0 1rem 0; display:none;"></div></div><div class="card-actions"></div>`;
    ...
    createListItem: function(item) {
        ...
        if(item.phones && item.phones.length > 0) actionsContainer.appendChild(this._createActionButton('fa-copy', () => { navigator.clipboard.writeText(item.phones[0].value); LifeOS.ui.showToast('تم نسخ الرقم'); }, 'نسخ', true));
        return li;
    },
    ...
    showForm: function(item = null) {
        ...
        form.addEventListener('submit', async (e)=>{
            ...
            const newItem={id:isEdit?item.id:LifeOS.core.generateId(), name:form.name.value, nickname:form.nickname.value, birthday:form.birthday.value, organization:{name:form.org_name.value,title:form.org_title.value}, phones: getFieldsData('phone',form.querySelector('#phone-fields-container')), emails:getFieldsData('email',form.querySelector('#email-fields-container')), websites:getFieldsData('website',form.querySelector('#website-fields-container')), notes:form.notes.value, photo:photoData, labels:form.labels.value.split(',').map(l=>l.trim()).filter(Boolean), created:isEdit?(item.created||Date.now()):Date.now(), updated:Date.now()};
            if(isEdit){const idx=LifeOS.core.state.data.contacts.findIndex(c=>c.id===item.id);if(idx>-1)LifeOS.core.state.data.contacts[idx]=newItem;}else{LifeOS.core.state.data.contacts.unshift(newItem);}
            await LifeOS.core.saveData();LifeOS.ui.closeModal();LifeOS.ui.showToast(isEdit?'تم تحديث الملف':'تم إضافة الملف','success');this.renderGrid();LifeOS.dashboard.load();
        });
        LifeOS.ui.showModal(isEdit ? 'تعديل الملف' : 'إضافة ملف جديد', form);
    },

    delete: async function(id) { if(confirm('هل أنت متأكد من حذف هذا الملف؟')){ LifeOS.core.state.data.contacts=LifeOS.core.state.data.contacts.filter(c=>c.id!==id);await LifeOS.core.saveData();LifeOS.ui.showToast('تم حذف الملف','success');this.renderGrid();LifeOS.dashboard.load();}},
    updateFilters: function() { const c=document.getElementById('contacts-filters'); c.innerHTML=''; const allTags=new Set(LifeOS.core.state.data.contacts.flatMap(i=>i.labels||[])); const createTag=tag=>{const t=document.createElement('div');t.className=`filter-tag ${tag===this.currentFilter?'active':''}`; t.dataset.filter=tag; t.textContent=tag==='all'?'الكل':tag; t.onclick=()=>{this.currentFilter=tag; this.renderGrid();}; return t;}; c.appendChild(createTag('all')); allTags.forEach(t=>c.appendChild(createTag(t)));},
    
    handleExternalImport: function(file) { if(!file)return;const r=new FileReader();r.onload=async e=>{try{const d=this.parseGoogleCSV(e.target.result);if(d.length===0){LifeOS.ui.showToast('لم يتم العثور على بيانات صالحة','warning');return;} const confirmed=await this.showImportConfirmation(d); if(confirmed){this.mergeImportedData(d);await LifeOS.core.saveData();LifeOS.ui.showToast(`تم دمج ${d.length} ملف بنجاح`,'success');this.renderGrid();LifeOS.dashboard.load();}}catch(err){console.error("Import failed:",err);LifeOS.ui.showToast('فشل معالجة الملف','error');}};r.readAsText(file,'UTF-8');},
    showImportConfirmation: function(data) { return new Promise(r=>{const c=document.createElement('div'); c.innerHTML=`<p class="mb-2">تم العثور على <strong>${data.length}</strong> ملف. هل تود دمجها؟</p><h4 class="mb-1 mt-3">عينة:</h4><div style="background:var(--bg-color);padding:0.8rem;border-radius:8px;font-size:0.9rem;max-height:150px;overflow-y:auto;">${data.slice(0,5).map(i=>`<div><strong>${LifeOS.core.sanitize(i.name)}</strong> - ${LifeOS.core.sanitize(i.phones?.[0]?.value||'N/A')}</div>`).join('')}</div><div class="flex gap-2 mt-3"><button class="btn btn-success confirm-btn" style="flex:1"><i class="fas fa-check"></i> تأكيد</button><button class="btn btn-danger cancel-btn" style="flex:1"><i class="fas fa-times"></i> إلغاء</button></div>`; c.querySelector('.confirm-btn').onclick=()=>{LifeOS.ui.closeModal();r(true);}; c.querySelector('.cancel-btn').onclick=()=>{LifeOS.ui.closeModal();r(false);}; LifeOS.ui.showModal('تأكيد استيراد',c);});},
    mergeImportedData: function(data) { data.forEach(item=>{item.id=LifeOS.core.generateId(); item.created=Date.now(); item.updated=Date.now(); if(!item.labels.includes('مستورد'))item.labels.push('مستورد');const exists=LifeOS.core.state.data.contacts.some(c=>c.name===item.name && c.phones?.[0]?.value===item.phones?.[0]?.value); if(!exists)LifeOS.core.state.data.contacts.push(item);});},
